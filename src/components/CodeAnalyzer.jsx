...
const CodeAnalyzer = ({ onAddXp, apiKey }) => {
    ...
    const exportReport = () => {
        ...
        markdown += `**Production Score:** ${score}/100 (${getScoreGrade().grade})\n`;
        markdown += `**Status:** ${getScoreGrade().text}\n\n`;
        markdown += `---\n\n`;
        markdown += `## Findings (${results.length} issues)\n\n`;

        results.forEach((issue, idx) => {
            markdown += `### ${idx + 1}. ${issue.title}\n\n`;
            markdown += `**Type:** ${issue.type.toUpperCase()}\n\n`;
        ...
        markdown += `\n*Report generated by DevMistakes v3.0 Pro Suite*\n`;
                        ...
                        <div style={{ display: 'flex', gap: '8px' }}>
                            <button
                                ...
                                style={{
                                    ...
                                    color: results && results.length > 0 ? 'var(--color-success)' : 'var(--text-dim)',
                                    fontSize: '0.75rem', fontWeight: 700, cursor: results && results.length > 0 ? 'pointer' : 'not-allowed',
                                    opacity: results && results.length > 0 ? 1 : 0.5,
                                    transition: 'all 0.2s'
                                }}
                            >
                                📥 Export Report
